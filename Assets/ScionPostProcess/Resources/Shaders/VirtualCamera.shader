...
{	    	
	...
	{		 
		...
		cameraOutput.CoCScaleAndBias = ComputeCoCScaleAndBias(fNumber, FocalLength, VCFocalDistance, CoCToPixels);
	...
	PackedCameraOutput AperturePriority(v2f i)
	{
		...
		cameraOutput.exposure = ClampExposure(ConvertEV100ToExposure(EV100) * LIGHT_INTENSITY_MULT);
		cameraOutput.CoCScaleAndBias = ComputeCoCScaleAndBias(ManualFNumber, FocalLength, VCFocalDistance, CoCToPixels);
		cameraOutput.sceneLuminance = sceneLuminance;
