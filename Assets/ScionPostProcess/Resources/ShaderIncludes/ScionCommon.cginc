...
uniform float4 _ScionCameraPosition;
#define CameraPosition float3(_ScionCameraPosition.xyz)
...
float UVBasedNoise01(float2 uv, float seed = 6272.3f)
{
...
float2 VignettedChromaticDistortionOffset(float2 uv, float scale) 
{
	float vignetteMask = 1.0f - VignetteMask(uv, scale);
	float offsetScale = vignetteMask * InvFullResSize;	
	return normalize(uv * 2.0f - 1.0f) * offsetScale;
}
